...
Two examples implemented in R (lots of visualisation packages):

  1. How to track and visualize NFT transactions: OpenSea and EtherScan APIs 
...
![](figures/glimpseWeirdWhales2.png){height="300px" fig-align="center"}
...
- Perfect data to be plotted as a network. We will display all the wallet addresses that have ever traded Weird Whales (restricted to tokens involved in at least 10 transactions).
...
- Helium: decentralized wireless infrastructure for IoT devices (environmental sensors, localisation sensors to track bike fleets,...), blockchain that leverages a decentralized global network of Hotspots
- People are incentivized to install hotspots and become a part of the network by earning Helium tokens 
- Lot of content available about the coverage aspect of Helium, but what about the real use ? :
  - How big is the Helium network, where are located the hotspots?
...
- Dewi ETL project: dedicated ETL server up and running. Interface (metabase) available to download extracts but limited to 10^6 rows. Fortunately, the team put CSV extracts in 10k/50k-block increments on a server: centralized, bad practice!

## Helium IoT network: hotspot data
...
- Every 24 bytes sent cost 1 Data Credit (DC) = $0.00001
...
- Median number of transactions per hotspot: 42 
...
- Blockchain: approaches for reading the blockchains and on how to visualize the blockchain transactions 
- NFT: We have shown an example of how to download and plot a network of transactions associated to NFTs. Analysing and plotting transactions is easy once you have the data. On the other hand, obtaining data in an appropriate format requires a deep understanding of the blockchain, especially when working with smart contracts.
- Helium: We have shown some techniques on how to summarise and visualise a network growth in term of infrastructure (hotspots) and data usage (transactions). We have analysed spatio-temporal data and have plotted them using dedicated R packages.
